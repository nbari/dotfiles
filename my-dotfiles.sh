...
X
X	# we enter repo to get local aliases as well.
X	builtin cd -q $dir
X
...
X	setopt localoptions noshwordsplit
X	local untracked_dirty=$1 dir=$2
X
...
X	prompt_pure_async_git_arrows $1
X}
X
...
X	unset MATCH MBEGIN MEND
X
X	async_job "prompt_pure" prompt_pure_async_vcs_info $PWD
X
...
X		async_job "prompt_pure" prompt_pure_async_git_aliases $working_tree
X	fi
X
X	async_job "prompt_pure" prompt_pure_async_git_arrows $PWD
X
...
X		async_job "prompt_pure" prompt_pure_async_git_fetch $PWD
X	fi
X
...
X		async_job "prompt_pure" prompt_pure_async_git_dirty ${PURE_GIT_UNTRACKED_DIRTY:-1} $PWD
X	fi
X}
X
