...
X  if s:is_win
X    set shell=cmd.exe shellcmdflag=/c shellredir=>%s\ 2>&1
X  elseif a:swap
...
X            \ 'batchfile': (s:is_win && (s:nvim || s:vim8)) ? tempname().'.bat' : '',
...
X  call s:bar()
X
X  if has_key(job, 'batchfile') && !empty(job.batchfile)
X    call delete(job.batchfile)
X  endif
...
Xendfunction
X
Xfunction! s:shellesc_cmd(arg)
X  let escaped = substitute(a:arg, '[&|<>()@^]', '^&', 'g')
X  let escaped = substitute(escaped, '%', '%%', 'g')
X  let escaped = substitute(escaped, '"', '\\^&', 'g')
X  let escaped = substitute(escaped, '\(\\\+\)\(\\^\)', '\1\1\2', 'g')
X  return '^"'.substitute(escaped, '\(\\\+\)$', '\1\1', '').'^"'
Xendfunction
X
Xfunction! s:shellesc(arg)
X  if &shell =~# 'cmd.exe$'
X    return s:shellesc_cmd(a:arg)
X  endif
