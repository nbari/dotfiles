...
X  let lod = { 'ft': {}, 'map': {}, 'cmd': {} }
X
X  if get(g:, 'did_load_filetypes', 0)
X    filetype off
X  endif
X  for name in g:plugs_order
X    if !has_key(g:plugs, name)
...
X  let sha = matchstr(getline('.'), '^  \X*\zs[0-9a-f]\{7,9}')
X  if empty(sha)
X    let name = matchstr(getline('.'), '^- \zs[^:]*\ze:$')
X    if empty(name)
X      return
X    endif
X    let title = 'HEAD@{1}..'
X    let command = 'git diff --no-color HEAD@{1}'
X  else
X    let title = sha
X    let command = 'git show --no-color --pretty=medium '.sha
X    let name = s:find_name(line('.'))
X  endif
X
X  if empty(name) || !has_key(g:plugs, name) || !isdirectory(g:plugs[name].dir)
...
X    execute 'e' title
X  else
X    execute 'pedit' title
X    wincmd P
X  endif
X  setlocal previewwindow filetype=git buftype=nofile bufhidden=wipe nobuflisted modifiable
X  let batchfile = ''
X  try
X    let [sh, shellcmdflag, shrd] = s:chsh(1)
X    let cmd = 'cd '.plug#shellescape(g:plugs[name].dir).' && '.command
X    if s:is_win
X      let [batchfile, cmd] = s:batchfile(cmd)
X    endif
X    execute 'silent %!' cmd
...
X  let anchor = line('$') - 3
X  let names = sort(keys(filter(copy(g:plugs),
X        \'has_key(v:val, "uri") && isdirectory(v:val.dir)')))
X  for name in reverse(names)
X    let sha = has_key(g:plugs[name], 'commit') ? g:plugs[name].commit : s:git_revision(g:plugs[name].dir)
X    if !empty(sha)
X      call append(anchor, printf("silent! let g:plugs['%s'].commit = '%s'", name, sha))
X      redraw
X    endif
...
X# zsh
X#set-option -g default-shell /bin/zsh
X#set-option -g default-command "reattach-to-user-namespace -l /bin/zsh"
X
