...
Xsetlocal wrap
Xsetlocal wrapmargin=0
Xlet s:l = 70 - ((19 * winheight(0) + 20) / 40)
Xif s:l < 1 | let s:l = 1 | endif
Xexe s:l
Xnormal! zt
X70
Xnormal! 016|
Xlcd ~/projects/rust/compound_duration
Xlet &so = s:so_save | let &siso = s:siso_save
Xdoautoall SessionLoadPost
X" vim: set ft=vim :
...
Xsetlocal wrap
Xsetlocal wrapmargin=0
Xlet s:l = 174 - ((8 * winheight(0) + 20) / 40)
Xif s:l < 1 | let s:l = 1 | endif
Xexe s:l
Xnormal! zt
X174
Xnormal! 042|
Xlcd ~/projects/rust/cron-parser
Xlet &so = s:so_save | let &siso = s:siso_save
Xdoautoall SessionLoadPost
X" vim: set ft=vim :
...
X      \ 'for /f "tokens=*" %%a in (''chcp'') do for %%b in (%%a) do set origchcp=%%b',
X      \ 'chcp 65001 > nul'
X    \ ]
X    \ + (type(a:cmds) == type([]) ? a:cmds : [a:cmds])
X    \ + ['chcp !origchcp! > nul', 'endlocal'],
X    \ 'v:val."\r"')
X  endfunction
X
...
X    let cmd = plug#shellescape(batchfile, {'shell': &shell, 'script': 1})
...
X    endif
X    if j.new
X      call s:system('rm -rf ' . plug#shellescape(g:plugs[name].dir))
X    endif
X  endfor
X  let s:jobs = {}
Xendfunction
...
X      let cmd = 'git log --graph --color=never '.join(map(['--pretty=format:%x01%h%x01%d%x01%s%x01%cr', range], 'plug#shellescape(v:val)'))
X      if has_key(v, 'rtp')
